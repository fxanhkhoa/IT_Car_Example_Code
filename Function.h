...
#define SERVO_CENTER		3000 +(22*1)		//Sai s? c?a c?n sensor trên xe
...
//Variable RATIO
#define ratio_default 0.6
uint16_t cnt_ratio, pulse_ratio;
uint16_t velocity;
float ratio;					//T? s? t?c ??
float ratio_base;				//T? s? t?c ?? n?n	
...
void speed(int left, int right,int percent)
{
	left  = left  *  ratio;
	right = right *  ratio;
	left = left *percent/100;
	right = right *percent/100;
	if(left>=0)
	{
		cbi(PORTD,DIR0);
		OCR1B=left*200;
...
void get_speed()
{
	...
	while(1)
	{
		...
		{
			speed += 0.05;
			if (speed > 1.05)				{ speed = ratio_base; }
			//else if (speed > 1)		{ speed = 1;}
			else if (speed > 1.01)		{ speed = 1; }
		}
		led7(speed*100);
